...
class User {
    ...
    async createUser() {
        ...
        if(clientInfo.nickname.length > 12){
            return { error : 'Bad Request', message : "닉네임이 12자 이상입니다.", statusCode : 400};
        }if(clientInfo.id.length > 15){
            return { error : 'Bad Request', message : "아이디가 15자 이상입니다.", statusCode : 400};
        }if(clientInfo.email.length > 30){
            return { error : 'Bad Request', message : "이메일이 30자 이상입니다.", statusCode : 400};
        }

        const users = await UserStorage.findUsers(clientInfo.id, clientInfo.nickname);
        ...
        const hashPassword = bcrypt.hashSync(clientInfo.password, salt);
            try {
                    await UserStorage.save(clientInfo.nickname, clientInfo.id, hashPassword, clientInfo.email);
